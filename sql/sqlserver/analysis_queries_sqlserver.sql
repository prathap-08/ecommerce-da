...
WITH monthly AS (
    ...
    GROUP BY DATEFROMPARTS(YEAR(o.order_date), MONTH(o.order_date), 1), MONTH(o.order_date)
)
SELECT
    month_start,
    month_num,
...
WITH monthly AS (
    ...
    FROM sales s
    JOIN orders o ON o.order_id = s.order_id
    GROUP BY MONTH(o.order_date), DATEFROMPARTS(YEAR(o.order_date), MONTH(o.order_date), 1)
)
SELECT
    month_num,
...
SELECT
    o.region,
    SUM(s.revenue) AS total_revenue,
    SUM(s.revenue - ISNULL(p.unit_cost, 0) * s.quantity) AS total_profit,
    ROUND(
        100.0 * SUM(s.revenue - ISNULL(p.unit_cost, 0) * s.quantity)
...
SELECT TOP 20
    o.customer_id,
    ISNULL(MAX(o.customer_name), o.customer_id) AS customer_name,
